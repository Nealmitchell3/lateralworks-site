    ...
    <>
      ...
      <section className="bg-navy pt-32 pb-20 lg:pt-40 lg:pb-24">
        <div className="max-w-8xl mx-auto px-6 lg:px-10">
          ...
          <h1 className="display-heading text-white text-5xl lg:text-6xl xl:text-7xl mb-4">
            {about.hero.headline}
          </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
            ...
            <div className="grid grid-cols-2 gap-px bg-border self-start">
              {[
                ...
                { number: "200+", label: "FTTM Projects" },
                { number: "36yr", label: "Research" },
                { number: "$7B+", label: "Accelerated" },
              ].map((item) => (
                <div key={item.label} className="bg-cream p-6">
                  <div className="stat-number text-4xl text-navy mb-1">{item.number}</div>
                  <div className="text-[11px] font-sans font-500 uppercase tracking-widest text-ink-muted">
                    {item.label}
                  </div>
                </div>
              ))}
            ...
            {team.members.map((member) => (
              <div key={member.name} className="bg-cream-dark p-8 card-hover">
                <div className="w-12 h-12 bg-navy flex items-center justify-center mb-5">
                  <span className="font-display text-xl font-500 text-white">
                    {member.name.charAt(0)}
                  </span>
                </div>
                <h3 className="font-sans font-600 text-base text-navy mb-1">{member.name}</h3>
                <p className="text-[11px] font-sans font-500 uppercase tracking-wider text-gold mb-3">
                  {member.role}
                </p>
        ...
        <div className="max-w-8xl mx-auto px-6 lg:px-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm font-sans font-300 text-white/50">
              Tell us about your program and let&apos;s see how FTTM can help.
            </p>
          </div>
          <Link
            href="/contact"
