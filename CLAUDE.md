...
2. **Ideas posts (dynamic)** — 191 articles in `content/posts/*.json` with `content/posts_index.json` as a sorted summary. `content/posts-utils.ts` exposes `getAllPostMeta`, `getPost`, `getAllSlugs`, `getPostsByPage`, `getRelatedPosts`. `app/ideas/page.tsx` is `ƒ` dynamic (filters via `?category=` and `?page=`). `app/ideas/[slug]/page.tsx` is `●` SSG via `generateStaticParams` over all slugs. Types in `content/post-types.ts`.

3. **Docs (dynamic)** — 115 release notes/how-tos in `content/docs/*.json` + `content/docs_index.json`. `content/docs-utils.ts` mirrors the posts API (`getAllDocMeta`, `getDoc`, `getDocsByPage`, `getRelatedDocs`). Same SSG pattern as /ideas. Types in `content/doc-types.ts`. Not linked from `Nav.tsx` yet — accessible by direct URL only.
